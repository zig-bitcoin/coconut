...
pub const MintKeyset = struct {
    private_keys: std.AutoHashMap(u64, SecretKey),
    public_keys: std.AutoHashMap(u64, PublicKey),
    keyset_id: [16]u8,
    mint_pubkey: PublicKey,

    pub fn init(allocator: std.mem.Allocator, seed: []const u8, derivation_path: []const u8) !MintKeyset {
        var priv_keys = try deriveKeys(allocator, seed, derivation_path);
        errdefer priv_keys.deinit();

        var pub_keys = try derivePubkeys(allocator, priv_keys);
        errdefer pub_keys.deinit();

        const keyset_id = try deriveKeysetId(allocator, pub_keys);

        const mint_pubkey = derivePubkey(allocator, seed) catch return error.InvalidSeed;
...
/// # Arguments
///
/// * `allocator` - A allocator that allocate result hash map.
...
pub fn deriveKeys(allocator: std.mem.Allocator, master_key: []const u8, derivation_path: []const u8) !std.AutoHashMap(u64, SecretKey) {
    var keys = std.AutoHashMap(u64, SecretKey).init(allocator);
    errdefer keys.deinit();

    var hash_buffer: [sha256.digest_length]u8 = undefined;
    // var usize_buf: [10]u8 = undefined;
    ...
    for (0..MAX_ORDER) |i| {
        ...
        try std.fmt.format(hasher.writer(), "{s}{s}{d}", .{ master_key, derivation_path, i });
        // sha256.init(.{});

        hasher.final(&hash_buffer);
        const key = try SecretKey.fromSlice(&hash_buffer);

        try keys.put(try std.math.powi(u64, 2, i), key);
    }
...
pub fn derivePubkeys(allocator: std.mem.Allocator, keys: std.AutoHashMap(u64, SecretKey)) !std.AutoHashMap(u64, PublicKey) {
    const secp = try Secp256k1.genNew(allocator);
    defer secp.deinit(allocator);

    var res = std.AutoHashMap(u64, PublicKey).init(allocator);
    errdefer res.deinit();
    var it = keys.iterator();

    while (it.next()) |e|
        try res.put(e.key_ptr.*, e.value_ptr.*.publicKey(secp));
...
fn deriveKeysetId(allocator: std.mem.Allocator, keys: std.AutoHashMap(u64, PublicKey)) ![16]u8 {
    ...
    while (it.next()) |k| {
        keys_arr.appendAssumeCapacity(k.*);
    }

    std.sort.block(u64, keys_arr.items, {}, std.sort.asc(u64));

    var hasher = sha256.init(.{});

    for (keys_arr.items) |k| {
        const pk = keys.get(k).?;
        hasher.update(&pk.serialize());
    }

    const hashed_pubkeys = std.fmt.bytesToHex(hasher.finalResult(), .lower);
...
pub fn parseMapOfPubkeys(allocator: std.mem.Allocator, json: []const u8) !std.AutoHashMap(u64, PublicKey) {
    ...
    while (true) {
        switch (try scanner.next()) {
            .string => |key| {
                const k = try std.fmt.parseInt(u64, key, 10);

                const pk = try std.json.innerParse(PublicKey, allocator, &scanner, .{});
...
pub fn derivePubkey(allocator: std.mem.Allocator, seed: []const u8) !PublicKey {
    var hasher = sha256.init(.{});
    hasher.update(seed);

    const hash = hasher.finalResult();
    const key = try SecretKey.fromSlice(&hash);

    const secp = try Secp256k1.genNew(allocator);
    defer secp.deinit(allocator);

    return key.publicKey(secp);
}

test "test_derive_pubkey" {
    const result = try derivePubkey(std.testing.allocator, "supersecretprivatekey");

    try std.testing.expectEqualSlices(u8, &result.toString(), "03a2118b421e6b47f0656b97bb7eeea43c41096adbc0d0e511ff70de7d94dbd990");
}

test "test_derive_keys_master_v1" {
    var keys = try deriveKeys(std.testing.allocator, "supersecretprivatekey", "");
    defer keys.deinit();

    try std.testing.expectEqual(64, keys.count());

    var pub_keys = try derivePubkeys(std.testing.allocator, keys);
    defer pub_keys.deinit();

    const id = try deriveKeysetId(std.testing.allocator, pub_keys);
    try std.testing.expectEqualSlices(u8, "00d31cecf59d18c0", &id);
}

test "test_derive_keyset_id" {
    const keys_json =
        \\{
    ...
    var pubs = try parseMapOfPubkeys(std.testing.allocator, keys_json);
    defer pubs.deinit();

    const keyset_id = try deriveKeysetId(std.testing.allocator, pubs);

    try std.testing.expectEqualSlices(u8, "00456a94ab4e1c46", &keyset_id);
}

test "test_derive_keyset_id_long" {
    const keys_json =
        \\{
