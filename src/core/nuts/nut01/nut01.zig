...
pub const KeysResponse = struct {
    ...
    pub fn jsonParse(allocator: std.mem.Allocator, source: anytype, options: std.json.ParseOptions) !KeysResponse {
        ...
        var arraylist = std.ArrayList(KeySet).init(allocator);
        while (true) {
            ...
            arraylist.appendAssumeCapacity(std.json.innerParse(KeySet, allocator, source, options) catch |e| if (e == std.mem.Allocator.Error.OutOfMemory) return e else {
                continue;
            });
        }
...
pub const Keys = struct {
    inner: std.StringHashMap(secp256k1.PublicKey),

    /// Get [`PublicKey`] for [`Amount`]
    pub inline fn amountKey(self: *const Keys, amount: u64) ?secp256k1.PublicKey {
        var buf: [21]u8 = undefined;

        const n = std.fmt.formatIntBuf(&buf, amount, 10, .lower, .{});

        return self.inner.get(buf[0..n]);
    }

    pub fn jsonParse(allocator: std.mem.Allocator, source: anytype, options: std.json.ParseOptions) !Keys {
        if (try source.next() != .object_begin) return error.UnexpectedToken;

        var result = std.StringHashMap(secp256k1.PublicKey).init(allocator);
        errdefer result.deinit();

        while (try source.peekNextTokenType() != .object_end) {
            switch (try source.next()) {
                .string, .allocated_string => |s| {
                    const val = try std.json.innerParse(secp256k1.PublicKey, allocator, source, options);

                    try result.put(s, val);
                },
                else => return error.UnexpectedToken,
            }
        }
    ...
    pub fn fromMintKeys(allocator: std.mem.Allocator, keys: MintKeys) !Keys {
...
/// Mint keys
pub const MintKeys = struct {
    inner: std.AutoHashMap(
        u64,
        MintKeyPair,
    ),
    ...
    pub inline fn initFrom(allocator: std.mem.Allocator, map: std.AutoHashMap(u64, MintKeyPair)) !MintKeys {
        return .{
            .inner = try map.cloneWithAllocator(allocator),
        };
    }
};

/// Mint Public Private key pair
pub const MintKeyPair = struct {
    /// Publickey
    ...
    pub inline fn fromSecretKey(secp: secp256k1.Secp256k1, secret_key: secp256k1.SecretKey) MintKeyPair {
        return .{
            .public_key = secret_key.publicKey(secp),
...
test "pubkey" {
    const pubkey_str = "02c020067db727d586bc3183aecf97fcb800c3f4cc4759f69c626c9db5d8f5b5d4";
    const pubkey = try secp256k1.PublicKey.fromString(pubkey_str);

    try std.testing.expectEqualSlices(u8, pubkey_str, &pubkey.toString());
}

test "test_ser_der_secret" {
    const secret = secp256k1.SecretKey.generate();
    ...
    const sec = try std.json.parseFromSlice(secp256k1.SecretKey, std.testing.allocator, output.items, .{});
    defer sec.deinit();

    try std.testing.expectEqual(secret, sec.value);
}

test "test_vectors_01" {
    const incorrect_1 =
        \\{
        ...
        \\}
    ;
    try std.testing.expect(if (std.json.parseFromSlice(Keys, std.testing.allocator, incorrect_1, .{})) |_| false else |_| true);
    ...
    const incorrect_2 =
        ...
        \\}
    ;
    try std.testing.expect(if (std.json.parseFromSlice(Keys, std.testing.allocator, incorrect_2, .{})) |_| false else |_| true);
    ...
    const correct_1 =
        ...
        \\}
    ;
    var val = try std.json.parseFromSlice(Keys, std.testing.allocator, correct_1, .{});
    defer val.deinit();
    ...
    const correct_2 =
        ...
        \\}
    ;
    var val1 = try std.json.parseFromSlice(Keys, std.testing.allocator, correct_2, .{});
    defer val1.deinit();
}
